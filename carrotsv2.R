...
#still need to implement supposedly the possible cells that constitutes as center...
...
#bunnies collecting carrots before falling asleep for not finding carrots, using while loop
newcarrots=1
while (newcarrots!=0){
  ...
  y = 2
    ...
    possdir <- list(c(x+1, y), c(x, y+1), c(x-1, y), c(x, y-1)) #possible jumping "coordinates", right, up, left, down
      ...
      #find the index within the created list of possible directions
      index_from_newcoord_list <- match(max(carrots_newcoord_list),carrots_newcoord_list)
      
      #next coordinate of the adjacent position, saved in x and y 
      x <- possdir[index_from_newcoord_list][1]
      y <- possdir[index_from_newcoord_list][2]
      
      newcarrots = max(carrots_newcoord_list)
